...
bool ExvisCommuCANAssem::runAssem() {
	if(ros::ok()){
		...
		comCAN->writeCAN(comCAN->msgPerAssistCAN(realRos->assist), 76);
		...
		switch (realRos->type) {
			...
			case 1 :	{
				...
				comCAN->writeCAN(comCAN->msgPositionConCAN(), 71); // type of control: Position
				comCAN->writeCAN(comCAN->msgStepPositionCAN(realRos->jointAngleCon), 72);
				break;
			...
			case 3 :	{
				...
				comCAN->writeCAN(comCAN->msgTorqueConCAN(), 71); // type of control: Torque
				comCAN->writeCAN(comCAN->msgStepTorqueCAN(realRos->torqueCon), 72);
				break;
			}
